...
export function* fetchProducts(action: FetchProductsAction): any {
  ...
  const products: IProductTemp[] = yield select((state: ReduxStore) => state.products.productsResponse.content);
  const { payload: { page, category, subCategory, search }} = action;
  const token = yield select((state: ReduxStore) => state.auth.token);
  try {
    const result = yield call(
      fetch,
      `${API_BASE_URL}/products/?${page ? `page=${page}` : 'page=1'}${category ? `&category=${category}` : ''}${subCategory ? `&subCategory=${subCategory}` : ''}${search ? `&search=${search}` : ''}`,
      {
        method: 'GET',
        headers: new Headers({
